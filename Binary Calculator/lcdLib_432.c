...
void lcdInit() {
	...
	P4DIR = EN + RS + DATA;  // Make pins outputs
...
void custom_char(unsigned char *myChar, int pos)
{
	int i;
	lcdWriteCmd(0x40 + (0x08 * pos));
	for(i = 0; i < 8; i++)
	{
		lcdWriteData(myChar[i]);
	}
}

void print_custom(int custom_num, int x, int y)
{
	if (x < 16) {
		x |= 0x80; // Set LCD for first line write
		switch (y){
